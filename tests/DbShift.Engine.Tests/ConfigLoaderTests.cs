...
public class ConfigLoaderTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "dbshift-config-" + Guid.NewGuid().ToString("N")[..8]);
    ...
    {
        Environment.SetEnvironmentVariable("DBSHIFT_TEST_CS", "Host=expanded;Database=test;");
        _envVarsToRestore.Add("DBSHIFT_TEST_CS");

        WriteMigrationConfig(@"{
            ""migration"": {
                ""database"": { ""connectionString"": ""${DBSHIFT_TEST_CS}"" }
            }
        }");
    ...
    {
        Environment.SetEnvironmentVariable("DBSHIFT_TEST_HOST", "envhost");
        _envVarsToRestore.Add("DBSHIFT_TEST_HOST");

        WriteEnvironmentConfig("qa", @"{
            ""database"": { ""host"": ""${DBSHIFT_TEST_HOST}"" }
        }");
